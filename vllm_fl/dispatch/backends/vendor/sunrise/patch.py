...
def apply_sunrise_patches():
    ...
    # GDN linear-attention kernel rebinds MUST run before model construction so
    # that fla.ops points at the PTPU sgl_kernel drop-ins (Qwen3.5/3.6 has 48
    # linear_attention layers whose recurrent kernel runs every decode step).
    # FlagGems pointwise fast-path: skip the fp64-fallback check that adds
    # ~4.3us/pointwise op on the bf16 hot path (~2-4ms/step on Qwen3.5).
    # Ported from 0.2.1; the 0.24 minimal port had dropped it. Must run
    # before any FlagGems op dispatch, hence first in the sequence.
    _apply_pointwise_fast_path()
    patch_gdn_kernels()
...
def patch_gdn_kernels():
    """Route vLLM FLA/GDN kernels to PTPU sgl_kernel (Qwen3.5/3.6 linear-attn).

    Ported from the 0.2.1 ``sunrise/patches`` package (error A / GDN Tier 0).
    Only the two GDN-relevant patches are applied here; MoE/int8/pointwise
    patches from the 0.2.1 master are intentionally left out (out of scope for
    the dense 27B model; reserved for the 35B-A3B MoE port).
    """
    try:
        ...
        from .patches import patch_fla_ops, patch_gdn_core_attn_buf

        patch_fla_ops.apply_patch()
        patch_gdn_core_attn_buf.apply_patch()
    except Exception:  # pragma: no cover - defensive, matches sibling patches
        logger.exception("Failed to apply PTPU GDN kernel patches")


def patch_flagcx_comm_lifecycle():
    """Defer flagcxCommInitRank to first collective; set persistent _device_ctx.

    Ported from 0.2.1 sunrise/patches/patch_flagcx_comm.py (expG). This is the
    dependency that expE/expF were MISSING: the stock 0.3.0rc0 __init__ eagerly
    creates the comm and runs a warmup all_reduce bound to vllm.utils
    current_stream() (stale on PTPU), and never sets self._device_ctx. This
    patch replaces __init__ with a lazy version that (1) leaves self.comm=None
    until the first real collective triggers _ensure_initialized(), and (2)
    stores self._device_ctx = torch_ptpu.device(self.device) so the collective
    hot path launches every kernel inside the correct PTPU device context.
    Apply order MUST be lifecycle -> stream_adapter -> hot_path.
    """
    try:
        from vllm.platforms import current_platform
        ...
        from .patches.patch_flagcx_comm import (
            patch_flagcx_comm_lifecycle as _patch,
        )

        _patch()
    except Exception as e:
        logger.warning("Failed to patch FlagCX comm lifecycle for Sunrise: %s", e)
...
def patch_flagcx_stream_adapter():
    ...
    try:
        ...
        def _adaptor_stream_copy(self, old_stream):
            # Cache flagcxStream_t per raw stream pointer and reuse it. Creating
            # + freeing a fresh stream on every collective mis-schedules the
            # pccl ring kernel under concurrency -> AllReduce kernel timeout.
        ...
        def _adaptor_stream_free(self, stream):
            # No-op: cached streams are reused for the lifetime of the comm.
            return
        ...
        logger.info("Patched FlagCX stream adapter for Sunrise/PTPU (cached + no-op free)")
    except Exception as e:
...
def patch_flagcx_collective_hot_path():
    ...
    Ported verbatim from 0.2.1 sunrise/patch.py. Self-contained: lazily creates
    self._device_ctx and guards _ensure_initialized with getattr(..., None), so
    it works against 0.3.0rc0's eager-init PyFlagcxCommunicator without the
    comm-lifecycle patch. Relies on the step-1 stream adapter (cached copy +
    no-op free) already installed by patch_flagcx_stream_adapter().

    The decisive fix vs the stock 0.3.0rc0 flagcx.py hot path: every kernel
    launch runs inside ``with torch.ptpu.device(self.device):`` and reuses a
    cached default-stream wrapper, instead of launching with no device context
    and allocating/copying a stream each step (the ~2min timeout trigger).
    """
    try:
        from vllm.platforms import current_platform
...
def patch_op_cls():
    ...
    # Ported from 0.2.1: register gemma/rms_norm_gated OOT (impl files now present).
    try:
        from .impl.gemma_rms_norm import register_gemma_rms_norm_oot

        register_gemma_rms_norm_oot()
    except Exception as e:
        logger.warning("Failed to register gemma_rms_norm OOT for Sunrise: %s", e)
    ...
    try:
        ...
        logger.warning("Failed to register rms_norm_gated OOT for Sunrise: %s", e)
...
def patch_ptpu_topk_topp_sampler():
    ...
    Ported from 0.2.1 sunrise/patch.py. The 0.24 default forward_native calls
    ``apply_top_k_top_p`` -> radix sort every decode step (confirmed via py-spy),
    which is slow on PTPU and inflates the per-rank step time, widening the TP
    timing drift that trips the pccl ring all_reduce kernel timeout. This routes
    the common (no per-request generator) path through the fused PTPU kernel.
    Adapted for 0.24: random_sample takes a third ``use_fp64_gumbel`` arg.
    """
    ...
    def _ptpu_forward_native(self, logits, generators, k, p):
        ...
        if k is None and p is None:
            ...
            return random_sample(probs, generators, use_fp64), None
...
def _apply_pointwise_fast_path():
    """Restore FlagGems PointwiseDynamicFunction fast path on PTPU (ported 0.2.1)."""
    try:
        from vllm.platforms import current_platform
        ...
        from .patches import patch_pointwise

        patch_pointwise.apply_patch()
    except Exception:  # pragma: no cover - defensive, matches sibling patches
        logger.exception("Failed to apply PTPU pointwise fast-path patch")


# ==== ported from 0.2.1 patch.py (all-patches port 2026-09-20) ====
