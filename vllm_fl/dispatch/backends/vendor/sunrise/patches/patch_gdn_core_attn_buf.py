...
def apply_patch() -> bool:
    ...
    # vLLM module layout for the GDN linear-attention layer differs by version:
    #   * 0.20.2: vllm...mamba.gdn_linear_attn.GatedDeltaNetAttention
    #   * 0.24.0: vllm...mamba.gdn.qwen_gdn_linear_attn.QwenGatedDeltaNetAttention
    #             (the base GatedDeltaNetAttention in mamba.gdn.base does NOT
    #              define forward_cuda; the Qwen subclass does — that is the
    #              class whose forward_cuda does the per-step torch.zeros we
    #              want to elide).
    # Probe the known locations in order and take the first that carries a
    # concrete forward_cuda + the num_v_heads/tp_size/head_v_dim attributes the
    # wrapper relies on.
    _CANDIDATES = (
    ...
    for _mod_name, _cls_name in _CANDIDATES:
        try:
            _mod = importlib.import_module(_mod_name)
        except Exception:  # module absent in this vLLM version
            continue
        _cls = getattr(_mod, _cls_name, None)
        if _cls is None:
            continue
        # forward_cuda must be defined on the class itself (not merely
        # inherited as an abstract stub) for the rebind to take effect.
        if "forward_cuda" not in _cls.__dict__:
            continue
        _gdn_mod = _mod
        GatedDeltaNetAttention = _cls
        break

    if _gdn_mod is None or GatedDeltaNetAttention is None:
        _log.warning(
            "Sunrise GDN core_attn_out patch skipped: no GDN linear-attention "
            "class with a concrete forward_cuda found among %s. Patch had no "
            "effect.",
            [f"{m}.{c}" for m, c in _CANDIDATES],
        )
        return False
